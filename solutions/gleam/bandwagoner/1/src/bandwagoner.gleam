...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    Team(_, coach, _) if coach.name == "Gregg Popovich" -> True
    Team(_, coach, _) if coach.former_player -> True
    Team(name, _, _) if name == "Chicago Bulls" -> True
    Team(_, _, stats) if stats.wins > 59 -> True
    Team(_, _, stats) if stats.losses > stats.wins -> True
    _ -> False
  }
}
